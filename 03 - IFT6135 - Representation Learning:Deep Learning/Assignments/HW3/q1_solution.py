...
def log_likelihood_normal(mu, logvar, z):
    ...
    *** note. ***
    ...
    return -0.5 * torch.sum(np.log(2*np.pi) + logvar + ((z - mu)**2 / torch.exp(logvar)), dim=1)
...
def log_mean_exp(y):
    ...
    *** note. ***
    ...
    # log_mean_exp
    maxy = torch.max(y, dim=1)[0]
    maxy_reshape = maxy.view(batch_size, -1)
    return maxy + torch.log(1.0 / sample_size * torch.sum(torch.exp(y - maxy_reshape), dim=1))
...
def kl_gaussian_gaussian_analytic(mu_q, logvar_q, mu_p, logvar_p):
    ...
    *** note. ***
    ...
    # kld
    return 0.5 * torch.sum(logvar_p - logvar_q - 1.0 + (torch.exp(logvar_q) / torch.exp(logvar_p) + ((mu_q - mu_p)**2) / torch.exp(logvar_p)), dim=1)
